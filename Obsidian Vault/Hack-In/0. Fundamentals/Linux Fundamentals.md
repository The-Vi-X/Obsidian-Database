...
- `diff [options] files` - команда для сравнения каждой строчки двух файлов и выявлений отличий #linux #command #text 
...
* `screen [options][command]`- запусть программы в новом терминале #linux #command #terminal
[[Взаимодействия с процессами]]fg`
* Ctrl+Z - Отправить процесс в фон и остановить его #linux #command
* `bg` - отправка процесса на задний фон
* `fg`- отправка процесса на передний фон
* `&`- после команды запускает процесс на заднем фоне
[[Удалённый доступ]]
